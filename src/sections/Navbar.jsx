import React, {useEffect, useState} from 'react'
import {navLinks} from "../constants/index.js";

const NavItems = () => {
    return (
        <ul className="nav-ul">
            {navLinks.map(({ id, href, name }) => (
                <li key={id} className="nav-li ">
                    <a href={href} className="nav-li_a block relative ">
                        <span className="nav-li__text block transition-transform duration-300 ease-in-out">{name}</span>
                    </a>
                </li>
            ))}
        </ul>
    );
}


// DO NOT FORGET TO BUTTON sm:hidden

const Navbar = () => {
    const [isOpen, setIsOpen] = useState(false);

    const toggleMenu = () => setIsOpen((prevIsOpen) => !prevIsOpen);

    return (
        <header className={"fixed top-0 left-0 right-0 z-50 bg-primary/50"}>
            <div className={"max-w-7xl mx-auto"}>
                <div className={"flex justify-between items-center py-3 mx-auto c-space"}>
                    <a href={"/"} className={"text-[#DDE6ED] font-bold text-xl hover:text-white transition-colors"}>Mathias Goris</a>

                    <button onClick={toggleMenu} className={"text-neutral-400 hover:text-white focus:outline-none sm:hidden flex"} aria-label={"Toggle menu"}>
                        <img src={isOpen ? "assets/close.svg" : "assets/menu.svg"} alt={"toggle"} className={"w-6 h-6"} />
                    </button>

                    <nav className={"sm:flex hidden"}>
                        <NavItems />
                    </nav>

                </div>
            </div>

            <div className={`nav-sidebar ${isOpen ? 'max-h-screen' : 'max-h-0'}`}>
                <nav className={"p-5"}>
                    <NavItems />
                </nav>
            </div>
        </header>
    )
}
export default Navbar
